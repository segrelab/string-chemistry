...
The adjusted R-squared value for this model is negative, which means it is aprroximately as bad as it can possibly be. The regression diagnostic plots indicate that the model is bad because: none of the true outcome points are negative, the large proportion of outcomes that are 0, and non-constant variance in the residuals.
...
Most biomass reactions are significant predictors of growth, which is nice, but this model still only accounts for about 36% of variation in growth fluxes. The regression diagnostic plots are a bit hard to interpret since we're using one categorical predictor, but it generally seems to have all the same problems as the environment model.
...
Most of the biomass predictions are significant predictors of growth, but those that weren't were very far from being significant. The Nagelkerke R-squared and C-statistic for this model are noticeably better than from the previous model: an R-squared of 0.39 is still pretty bad, but an AUC of 0.856 is actually okay.
